        ...
        try:
            ...
            if color != "Unknown":
                if first_run:
                    ...
                    send_telegram_image(snap, caption=f"🚀 XAUUSDc Bot Started | Candle: {color} | {color_to_signal(color)}")
                    last_color = color
                    first_run = False
                    ...
                    if pending_color is None:
                        ...
                        snap = draw_and_save_snapshot(chart, x1, x2, color)
                        send_telegram_image(snap, caption=f"🔔XAUUSDc Candle changed: {color} | {color_to_signal(color)}")
                        last_color = color
                        pending_color = None
                    else:
